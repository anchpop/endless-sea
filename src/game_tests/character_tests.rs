...
pub(crate) fn create_world() {
    Test {
        ...
        check: |app, (character_id, initial_character_translation)| {
            let character = app.world.get::<Transform>(character_id).unwrap();
            assert!(character.translation.y == initial_character_translation.y);
        },
    }
    .run()
}

pub(crate) fn character_dies() {
    Test {
        setup: |app| {
...
pub(crate) fn sword_attack() {
    Test {
        ...
        // again.
        frames: (item::Item::Sword.cooldown().as_secs_f32() * TEST_FPS).ceil()
            as u64
            + 5,
        check: |app, object_id| {
            app.world
